...
| 1.6 | Sidebar shows correct routes for admin users | `verified` | Role-branched routes in `app/(dashboard)/_components/sidebar-routes.tsx`, role from `lib/roles-client.ts` | |
| 1.7 | Admin layout protects routes | `partial` | `app/(dashboard)/(routes)/admin/layout.tsx` redirects non-admins, but the guard is per-layout rather than centralised, and `lib/roles.ts` still grants ADMIN via `TEACHER_ID` | [ADR 0001](../adr/0001-identity-authentication-and-rbac.md), [#42](https://github.com/akomapahealth/akomapa-lms/issues/42) |
...
| 1.9 | Admin button in navbar appears for admin users | `verified` | Role-conditional rendering in `components/shell/app-header.tsx` | |
...
| 5.9 | Case study player renders scenarios and choices | `verified` | `app/(course)/courses/[courseId]/chapters/[chapterId]/_components/case-study-player.tsx` | |
