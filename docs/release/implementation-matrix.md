...
| 1.7 | Admin layout protects routes | `verified` | `TEACHER_ID` is retired and every page under `app/(dashboard)/(routes)/admin/` declares its own capability through `lib/auth/page.ts`; the layout no longer decides on their behalf. Route handlers still hold the deprecated `lib/roles.ts` adapter | [ADR 0001](../adr/0001-identity-authentication-and-rbac.md), [docs/permission-matrix.md](../permission-matrix.md), [#42](https://github.com/akomapahealth/akomapa-lms/issues/42) |
